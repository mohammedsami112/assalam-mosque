...
const fetchPostLoading = ref(true)
...
const getPost = () => {
  PostsApi.getPost(router.currentRoute.value.params.id).then(response => {
    post.value = response.data
    inputs.item_id = post.value.id
    inputs.title = post.value.title
    inputs.body = post.value.content
    previewThumbnail.value = post.value.thumbnail
    inputs.status = post.value.status
    inputs.category = post.value.post_category.id
  }).finally(() => {
    fetchPostLoading.value = false
  })
}
...
<template>
  <VCard :loading='fetchPostLoading'>
      ...
      <VForm @submit.prevent="updatePost()">
        <VRow>
          ...
          <VCol cols='12' lg='6'>
            ...
            ></VFileInput>
            <img style='width: 100%; border-radius: 5px' class='mt-3' :src="previewThumbnail">
          </VCol>
